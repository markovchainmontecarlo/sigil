...
Codex subscription policy can include concurrency, percentage quantum, reserve floor, capacity polling, and rearm policy. Codex metered admission can limit starts, runtime, or tokens; its operation reservation limits tokens. Claude metered admission limits starts or observed USD, and every operation requires a hard USD limit. Provider-managed subscription overage is provider policy, not selection of a Sigil metered profile.

State reports profile activity, active assignments, circuit state, and Codex reserved headroom. Metered profiles also report their bounded token usage. Reservations bind a profile to a process owner until cleanup. Circuits prevent admission until eligible recovery and, where required, `rearm`. Codex subscription status reads live provider capacity and reports remaining, reserved, and available percentages. Authentication may be `unknown`; Claude subscription capacity remains `unknown` because no stable probe supplies a percentage.
...
Registry corruption, unsupported versions, unsafe permissions, unverifiable locks or owners, unresolved credential sources, stale reservations, exhausted budgets, open circuits, unavailable profiles, authentication failures, capacity exhaustion, and unaccepted prompts fail closed with classified errors. Public failure projections expose only classification, disposition, fingerprint, and safe operation or RPC metadata.
