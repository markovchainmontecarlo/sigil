...
State reports a ledger, active assignments, circuit state, and Codex reserved headroom. Reservations bind a profile to a process owner until cleanup. Circuits prevent admission until eligible recovery and, where required, `rearm`. Capacity evidence kinds include `unknown`, `metered`, `above-floor`, and `at-or-below-floor`. Authentication may be `unknown`; Claude subscription capacity remains `unknown` because no stable probe supplies a percentage.
