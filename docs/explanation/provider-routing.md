...
Each run writes one redacted effective-configuration snapshot at its root. Assignment, capacity, prompt delivery and acceptance, usage, release, and classified failure events contain allowlisted metadata only. Recovery state persists only the identity needed to resume or settle ownership; telemetry is not another recovery authority.
