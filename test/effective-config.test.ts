...
describe("effective configuration", () => {
  test("attributes command, project, and default leaves by precedence", () => {
    ...
    expect(effective.values["plan.semanticReviewLimit"]).toMatchObject({ value: 2, source: "project" });
  });
  ...
  test("loadConfig remains the value-only projection and validates references", () => {
    ...
    const invalid = repoWithConfig({ ...project, plan: { planners: ["missing"], synthesizer: "worker", reviewer: "worker", semanticReviewLimit: 2 } });
